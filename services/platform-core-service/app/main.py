...
from .models import Permission, PlatformUser, RefreshToken, Role, RolePermission, SecurityAuditEvent, UserRole
from .schemas import LoginIn, PasswordChangeIn, RefreshIn, UserCreateIn
from .security import bearer_claims, hash_password, issue_access_token, new_refresh_token, token_hash, verify_password
...
def login(payload: LoginIn, session: Session = Depends(db)):
    user = session.scalar(select(PlatformUser).where(PlatformUser.username_normalized == payload.username.lower()))
...
def create_user(payload: UserCreateIn, request: Request, session: Session = Depends(db)):
    ...
    if "*" not in claims["permissions"] and "platform.users.create" not in claims["permissions"]: raise HTTPException(403, "permission denied")
