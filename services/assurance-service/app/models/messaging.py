"""Messaging + append-only audit for the Assurance Service."""
import uuid
...
class OutboxEvent(Base):
    __tablename__ = "ass_outbox_events"
    __table_args__ = (Index("ix_ass_outbox_state", "published_at", "attempts"),)
...
class InboxMessage(Base):
    __tablename__ = "ass_inbox_messages"
    __table_args__ = (Index("ix_ass_inbox_dedup", "consumer", "event_id"),)
...
class AuditLog(Base):
    __tablename__ = "ass_audit_log"
    __table_args__ = (Index("ix_ass_audit_tenant", "tenant_id"),)
...
class AsyncTask(Base, UuidPk):
    __tablename__ = "ass_async_tasks"
    __table_args__ = (Index("ix_ass_task_dedup", "tenant_id", "task_type", "idempotency_key"),)
