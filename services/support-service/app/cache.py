"""Best-effort Redis/Valkey helpers for the support service.

Redis is used ONLY for safe caching, short-lived advisory locks, rate limiting
and workload counters. It is never authoritative for tickets, SLA deadlines,
escalations, assignments, comments, audit events or communication. Every
failure fails open (no security bypass, no data loss).
"""
...
def workload_key(agent_id: str) -> str:
    return f"workload:{agent_id}"
