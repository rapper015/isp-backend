"""Business calendar arithmetic for SLA timers.

SLA working time is measured in *business seconds* against a tenant business
calendar: working hours per weekday, holidays, and a timezone. All functions
are pure and deterministic; the database stores the resulting deadlines.
"""
...
MAX_WORKING_DAYS = 730  # safety bound for deadline computation
...
def working_intervals(working_hours: dict, day: date, tz) -> list[tuple[datetime, datetime]]:
    """Aware working intervals for a single calendar day, or [] if closed."""
    ...
    for start_s, end_s in slots:
        ...
        # "24:00" is not a valid time-of-day; treat it as end of day.
        if end_s == "24:00":
...
def business_seconds_between(
    ...
    """Business seconds that elapse between two instants under the calendar."""
    # Normalize to the calendar timezone first so naive DB datetimes compare safely.
...
def deadline_after(
    ...
    """The instant exactly `business_seconds` of working time after `start`.

    Returns an aware datetime (UTC-normalized for storage). If the calendar has
    no working hours at all, falls back to wall-clock addition."""
    ...
    while guard <= MAX_WORKING_DAYS:
        ...
        day += timedelta(days=1)
    # Safety fallback: no usable working time found; use wall clock.
    return _utc(_as_tz(start, tz) + timedelta(seconds=business_seconds))


def default_working_hours() -> dict:
    """Mon-Fri 09:00-18:00, Sat 10:00-14:00, Sun closed (documented default)."""
    return {
        "mon": [["09:00", "18:00"]],
        "tue": [["09:00", "18:00"]],
        "wed": [["09:00", "18:00"]],
