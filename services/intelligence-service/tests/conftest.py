"""Hermetic test environment for the Intelligence Service (Milestone 10)."""
import os
import sys
...
os.environ.setdefault("DATABASE_URL", "sqlite:///./test_intelligence.db")
os.environ.setdefault("INTELLIGENCE_INTERNAL_API_KEY", "test-internal-key")
os.environ.setdefault("INTELLIGENCE_JWT_SECRET", "test-intelligence-secret-0123456789abcdef0123456789abcdef")
...
def _database():
    for path in ("test_intelligence.db", "intelligence.db"):
    ...
    yield
    for suffix in ("", "-wal", "-shm"):
        full = "test_intelligence.db" + suffix
        if os.path.exists(full):
            try:
                os.remove(full)
            except (PermissionError, FileNotFoundError):
...
def make_token(role: str = "PLATFORM_ADMIN", tenant: uuid.UUID | None = None,
    ...
    return jwt.encode({k: v for k, v in claims.items() if v is not None},
                      os.environ["INTELLIGENCE_JWT_SECRET"], algorithm="HS256")
...
@pytest.fixture
def ai_headers(tenant_id):
    return {"Authorization": f"Bearer {make_token('AI_ENGINEER', tenant_id)}"}
