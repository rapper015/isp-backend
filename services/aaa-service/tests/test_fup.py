...
from app.models import Credential, Nas, RadiusCommand, Tenant, UsageProjection
...
def test_fup_activation_is_idempotent_on_usage_threshold():
    ...
    tenant = Tenant(name="fup", policy={"default_policy": {"fup_threshold_bytes": 10, "fup_policy": {"upload_kbps": 100, "download_kbps": 200}}}); session.add(tenant); session.flush()
